...
void grBigBlue_801E93D8(Ground_GObj* gobj)
{
    ...
    /* This used to be read at raw console offsets from the Ground, which on
     * the host land in its header rather than in u. */
    struct grBigBlue_FlyerVars* flyer = &gp->u.bigblue.flyer;
...

    {
                    ...
                    ->u.bigblue.manager.event_data[1] != NULL)
            {
                ...
                if (count <= 1) {
                    ...
                    if (height != -3.4028235e38f) {
                        ...
                        HSD_JObjSetTranslate(jobj, &pos);
                        flyer->target_y = pos.y;
                        flyer->speed = yakumono_param->xD0;
                        HSD_JObjClearFlagsAll(jobj, JOBJ_HIDDEN);
                        flyer->state = 1;
                    }
                }
        ...
        case 3:
            ...
            case 1:
                if (pos.x > 0.0f) {
                    flyer->speed = 0.0f;
                    flyer->timer = (s32) yakumono_param->xD8;
                    flyer->state = 2;
                } else {
                    f32 range_scale;
                    f32 range;
            ...
            case 3:
                if (pos.x > (50.0f + Stage_GetBlastZoneRightOffset())) {
                    ...
                    flyer->speed = 0.0f;
                    {
                        Ground* mgp2 = GET_GROUND(Ground_GetMapGObj(32));
                        mgp2->u.bigblue.manager.event_data[1] = NULL;
                        if (jobj != NULL) {
                            mgp2->u.bigblue.manager.event_extra = jobj;
                        }
                        mgp2->u.bigblue.manager.event_data[0] = (void*) 1;
                    }
                    flyer->target_rot = 0.0f;
                    HSD_JObjSetRotationZ(jobj, 0.0f);
                    flyer->state = 0;
                }
                break;
            }
            {
                ...
                target_z = euler.z;
                flyer->target_rot = target_z;
                if (HSD_JObjGetRotationZ(jobj) < flyer->target_rot) {
                    f32 target;
                    f32 delta =
                        0.017453292f *
                        (yakumono_param->xD4 *
                         (flyer->target_rot - HSD_JObjGetRotationZ(jobj)));
                    HSD_JObjAddRotationZ(jobj, delta);
                    if (HSD_JObjGetRotationZ(jobj) >=
                        (target = flyer->target_rot))
                    {
                ...
                if (bound_y <= check_h) {
                    if (check_h == -3.4028235e38f) {
                        flyer->target_y = fwd.y;
                    } else {
                        flyer->target_y = check_h + yakumono_param->xCC;
                    }
                } else {
                    flyer->target_y = bound_y + yakumono_param->xCC;
                }

                diff_y = pos.y - flyer->target_y;
                if (diff_y < 0.0f) {
                    diff_y = -diff_y;
                }
                if (diff_y < 0.5f) {
                    vy = 0.0f;
                } else if (pos.y < flyer->target_y) {
                    vy = (flyer->target_y - pos.y) / yakumono_param->xE4;
                    if (vy > yakumono_param->xE8) {
                        vy = yakumono_param->xE8;
                    }
                } else {
                    vy = (flyer->target_y - pos.y) / yakumono_param->xEC;
...
void grBigBlue_801ECB50(Ground_GObj* gobj)
{
    ...
    /* Count active cars (not in state 1, 7, or 8).  This block and the two
     * below used to read the lanes at raw console offsets, and to set a
     * state only in MWCC inline assembly, so on the host it never changed. */
    {
        s32 lane;
        for (lane = 0; lane < 4; lane++) {
            u32 st = gp->u.bigblue.car.lanes[lane].state;
            if (st != 1 && st != 7 && st != 8) {
                active_count++;
            }
        }
    ...
    /* Find closest car */
    {
        ...
        if (found_ten == 0 && closest_lane != -1) {
            ...
            target_car->state = 10;
            for (lane = 0; lane < 4; lane++) {
                u32 st = lanes[lane].state;
                if ((st == 7 && lanes[lane].pos.x < target_car->pos.x) ||
                    (st == 8 && lanes[lane].pos.x > target_car->pos.x))
                {
                    lanes[lane].state = 4;
                }
            }
...
{
    /* The car manager's u.bigblue.car.  This used to be read through a union
     * of raw console offsets from the start of the Ground, with the lane
     * state taken from its bits as MWCC packs them. */
    Ground* ground = gobj->user_data;
    struct grBigBlue_CarVars* car = &ground->u.bigblue.car;
    struct grBigBlue_CarLane* this_lane;
    Point3d sp_vec;
    HSD_JObj* jobj;
    s32 active;
    s32 behind;
    ...
    /* Setup per-lane data */
    this_lane = &car->lanes[lane];

    {
        jobj = car->collision_jobjs[this_lane->collision_slot];
    }

    f31_rot = HSD_JObjGetRotationZ(jobj);

    {
        u32 state = this_lane->state;
        f32 target;
        ...
        if (state != 7 && state != 8) {
            ...
            for (idx = 0; idx < 4; idx++) {
                u32 st = car->lanes[idx].state;
                if (st != 1 && st != 7 && st != 8) {
                    active++;
                    if (idx != lane &&
                        car->lanes[idx].pos.x < this_lane->pos.x)
                    {
                        behind++;
                    }
                }
        ...
        /* Convergence step for position index */
        {
            ...
            current = this_lane->target;
            diff = target - current;

            if (diff < -0.5F) {
                this_lane->target = current - 0.5F;
            } else if (diff > 0.5F) {
                this_lane->target += 0.5F;
            } else {
                this_lane->target = target;
            }
        }
    }
    ...
    {
        this_lane->velocity += this_lane->accel;

        /* Clamp velocity to +-max_speed*scale */
        {
            if (this_lane->velocity >
                yakumono_param->x24 * Ground_801C0498())
            {
                this_lane->velocity =
                    yakumono_param->x24 * Ground_801C0498();
            } else if (this_lane->velocity <
                       -(yakumono_param->x24 * Ground_801C0498()))
            {
                this_lane->velocity =
                    -(yakumono_param->x24 * Ground_801C0498());
        ...
        {
            this_lane->delta += this_lane->velocity;
            this_lane->pos.x =
                this_lane->target + this_lane->delta;
        }
    }

    /* Height/bounce simulation */
    if (!(this_lane->direction)) {
        this_lane->gravity +=
            yakumono_param->x48 * Ground_801C0498();
        this_lane->height += this_lane->gravity;

        if (this_lane->height > 0.0F) {
            this_lane->height = 0.0F;
            this_lane->gravity = 0.0F;
        } else {
            if (this_lane->height <
    ...
    this_lane->amplitude +=
        yakumono_param->x3C * (heading_val - this_lane->amplitude);

    /* Rotation angle update */
    this_lane->rotation += 0.017453292F * yakumono_param->x40;
    if (this_lane->rotation > M_TAU) {
        this_lane->rotation =
            (f32) ((f64) this_lane->rotation - M_TAU);
    }

    heading_osc =
        this_lane->amplitude * sinf(this_lane->rotation);

    /* Compute ground reference level */
    rank_factor = (this_lane->pos.y -
                   yakumono_param->x2C * Ground_801C0498()) -
                  this_lane->height - heading_osc;

    /* Collision/grounding check */
    if (this_lane->direction) {
        /* Grounded path */
        ground_y = grBigBlue_801EC58C(&this_lane->pos, &sp_vec,
                                      (f32) 500.0f);

        if (-F32_MAX != ground_y && ground_y > rank_factor) {
            if (this_lane->angular_velocity < 0.0F) {
                this_lane->direction = 0;
        ...
        this_lane->pos.y += this_lane->angular_velocity;
        ...
        /* Track reference adjustment */
        {
            ...
            HSD_ASSERT(3256, gp2);
            this_lane->pos.y += gp2->u.bigblue.road.position.y -
        ...
        ground_y =
            grBigBlue_801EC58C(&this_lane->pos, &sp_vec, 20.0F * s);
...
heading_converge:
    ...
    HSD_JObjSetTranslate(jobj, &this_lane->pos);
...
{
    /* The car manager's lanes.  This used to be read through a union of raw
     * console offsets from the start of the Ground, as grBigBlue_801ED694
     * did. */
    Ground* ground = gobj->user_data;
    struct grBigBlue_CarLane* cars = ground->u.bigblue.car.lanes;
    ...
    case 10: {
        ...
        if (cars[index].pos.x > 0.0F) {
            if ((abs_speed = cars[index].velocity) < 0.0F) {
                abs_speed = -abs_speed;
            }
            scale = Ground_801C0498();
            norm = abs_speed / (yakumono_param->x28 * scale);
            if (((void) norm, (f32) cars[index].threshold) > norm) {
                scale = Ground_801C0498();
                cars[index].accel = -(yakumono_param->x28 * scale);
            } else {
                scale = Ground_801C0498();
                cars[index].accel = yakumono_param->x28 * scale;
            }
    ...
    case 2: {
        ...
        norm = abs_speed / (yakumono_param->x28 * scale);
        if (((void) norm, (f32) cars[index].threshold) > norm) {
            if (cars[index].pos.x < Stage_GetCamBoundsRightOffset())
            {
                scale = Ground_801C0498();
                cars[index].accel = yakumono_param->x28 * scale;
                break;
    ...
    case 3: {
        ...
        norm = abs_speed / (yakumono_param->x28 * scale);
        if (((void) norm, (f32) cars[index].threshold) > norm) {
            if (cars[index].pos.x > Stage_GetCamBoundsLeftOffset()) {
                scale = Ground_801C0498();
                cars[index].accel = -(yakumono_param->x28 * scale);
                break;
            }
    ...
    case 4: {
        ...
        target = cars[index].target;
        abs_speed = cars[index].velocity;
        (void) abs_speed;
        if (abs_speed < 0.0F) {
            abs_speed = -abs_speed;
        }
        ...
        if (target > cars[index].pos.x) {
            if (cars[index].velocity < 0.0F) {
                scale = Ground_801C0498();
                cars[index].accel = yakumono_param->x28 * scale;
            } else {
                scale = Ground_801C0498();
                predicted =
                    cars[index].velocity * norm +
                    cars[index].pos.x -
    ...
    case 5:
    case 6: {
        f32* alpha = &cars[index].alpha;

        cars[index].accel = 0.0F;
        *alpha += (1.0F / 60.0F);
        if (*alpha > 1.0F) {
            *alpha = 1.0F;
        }
        Ground_801C5630(ground, index, *alpha);
        break;
    }
    ...
    case 9: {
        cars[index].alpha -= (1.0F / 60.0F);
        if (cars[index].alpha < 0.0F) {
            cars[index].alpha = 0.0F;
        }
        Ground_801C5630(ground, index, cars[index].alpha);
        break;
    }
    }
...
void grBigBlue_801EF424(Ground_GObj* gobj)
{
    ...
    while (k < 10 && changed != 0) {
        ...
        /* The lanes used to be walked at raw console offsets. */
...
#define Gr_Fzero_Car_Max 30
    ...
    /* The lanes used to be walked at raw console offsets. */
    for (i = 0; i < 4; i++) {
        struct grBigBlue_CarLane* lane = &gp->u.bigblue.car.lanes[i];
        if (lane->collision_slot == car_num) {
            lane->gravity = -coll->x50 * params->x44;
        }
    }
}
...
struct lbColl_80008D30_arg1* grBigBlue_801EFC0C(enum_t arg)
{
    return NULL;
}
